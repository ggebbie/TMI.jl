...
controls = TMI.setup_inversion(γ;
    ...
    source = (prior = q₀,),
    mass_fraction = (prior = m0, variance = 1.0^2)
)
...
"""
Sets up and runs the L-BFGS optimization for the TMI inversion using Fminbox for constraints.
"""
function total_constrained_inversion(controls::ControlParameters, c_obs, γ::Grid; 
    ...
    #initialize the preconditoner with scaling from the priors
    scales = vcat([fill(var(x), length(x)) for x ∈ vec.([controls.boundary.u₀, controls.source.q₀, controls.massfrac.m₀])]...)
    P = Diagonal(1 ./scales)

    # Run the L-BFGS optimization with bounds.
    result_opt_fg = Optim.optimize(
        Optim.only_fg!(fg!), controls.lower_bound, controls.upper_bound, x0,
        Fminbox(LBFGS(;m = 15, #lucky number? 
                alphaguess = LineSearches.InitialHagerZhang(), 
                linesearch = LineSearches.BackTracking(), 
                scaleinvH0 = false,
                P = P)),
        Optim.Options(f_abstol = 1e-12, g_tol = 1e-8,
                    iterations = 10, outer_iterations = 2, 
                    store_trace = true,
                    show_trace = false, show_warnings = false));
    return result_opt_fg
end

# Example of how to run the inversion:
#
# 1. Construct the initial guess vector from the priors.
x0 = vcat([vec(controls.boundary.u₀), vec(controls.source.q₀), vec(controls.massfrac.m₀)]...)

#
# 2. Run the inversion.
result = total_constrained_inversion(controls, c_obs, γ; x0 = x0); 
#
# 3. To benchmark:
# @btime total_constrained_inversion(controls, c_obs, γ; x0 = $x0)