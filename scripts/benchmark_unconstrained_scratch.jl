...
function make_unconstrained_scratch(controls::ControlParameters, γ)
    ...
    du = deepcopy(controls.ub)
    dq = deepcopy(controls.uq)
    gdu = zero(deepcopy(controls.ub))
    gdq = zero(deepcopy(controls.uq))
...
controls = ControlParameters(; ub = ub, uq = uq, m = m0, u₀ = u₀, q₀ = q₀, m₀ = m0, Qᵤ = Qᵤ, Qₛ = Qₛ, Qₘ = Qₘ)
