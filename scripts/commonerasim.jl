...
#this ends up being silly - initial surface cond are 0 for anom file, so it just ends up being zero everywhere, but I left it there so i know how to do it in the future 
c0 = tracerinit(γ.wet)
c0[γ.wet] = Alu\bc[begin, :, :, :][γ.wet]
u0 = c0[γ.wet]

tspan = (tsfc[begin], tsfc[2])#tspan must occur within tsfc 
