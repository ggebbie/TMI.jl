...
function get_above_cube_boundary_condition(xlim, ylim, zlim, γ::Grid)
    ...
    kidx       = findlast(cube_flat .> 0)
    fld        = Field(A .* γ.wet, γ, :nothing, "upper boundary condition", "nothing")
    return TMI.getboundarycondition(fld, 3, kidx, γ)
end
...
function get_below_cube_boundary_condition(xlim, ylim, zlim, γ::Grid)
    ...
    A          = Float64.(cube_down)
    cube_flat  = sum(A, dims=(1,2))[:]
    kidx       = findfirst(cube_flat .> 0)
    fld        = Field(A  .* γ.wet, γ, :nothing, "lower boundary condition", "nothing")
    return TMI.getboundarycondition(fld, 3, kidx, γ)
end
...
function get_east_of_cube_boundary_condition(xlim, ylim, zlim, γ::Grid)
    ...
    lonidx     = findlast(cube_flat .> 0)
    fld        = Field(A .* γ.wet, γ, :nothing, "eastern boundary condition", "nothing")
    return TMI.getboundarycondition(fld, 1, lonidx, γ)
end
...
function get_west_of_cube_boundary_condition(xlim, ylim, zlim, γ::Grid)
    ...
    A          = Float64.(cube_west)
    cube_flat  = sum(A, dims=(2,3))[:]
    lonidx     = findfirst(cube_flat .> 0)
    fld        = Field(A  .* γ.wet, γ, :nothing, "western boundary condition", "nothing")
    return TMI.getboundarycondition(fld, 1, lonidx, γ)
end
...
function get_south_of_cube_boundary_condition(xlim, ylim, zlim, γ::Grid)
    ...
    latidx     = findlast(cube_flat .> 0)
    fld        = Field(A .* γ.wet, γ, :nothing, "southern boundary condition", "nothing")
    return TMI.getboundarycondition(fld, 2, latidx, γ)
end
...
function get_north_of_cube_boundary_condition(xlim, ylim, zlim, γ::Grid)
    ...
    cube_flat  = sum(A, dims=(1,3))[:]
    latidx     = findfirst(cube_flat .> 0)
    fld        = Field(A .* γ.wet, γ, :nothing, "northern boundary condition", "nothing")
    return TMI.getboundarycondition(fld, 2, latidx, γ)
end