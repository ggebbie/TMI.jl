...
using LinearAlgebra, SparseArrays, NetCDF, Downloads,
    ...
    GeoMakie, GLMakie, CairoMakie, ColorSchemes, Proj4
...
"""
    function planview(c::Field{T},depth,lon₀ = 220)::Array{T,2} where T <: Real

# Arguments
- `c::Field{T}`: tracer field
- `depth`: depth of interest, must be on grid
- `lon₀`: central longitude of planview, default=140 West
"""
function planview(c::Field{T},depth,lon₀ = 0) where T <: Real
...
"""
function planviewplot(c::Field{T}, depth, lims; lon₀ = 0, clabel = "tracer", titlelabel="planview plot") where T <: Real
