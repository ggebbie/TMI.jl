"""
    optim_fg_unconstrained_global_costfunction!(F, G, control_vector, ...)
    Interface for `Optim.jl` with unconstrained controls and optional rescaling.
# Arguments
...
"""
function optim_fg_unconstrained_global_costfunction!(F::Union{Nothing, Float64}, 
    ...
    state = unconstrained_global_forward(controls, c_obs, γ; locs = locs, debug = debug)
...
"""
    ...
    Forward pass: apply controls, solve tracer field, and accumulate cost components.
# Arguments
...
"""
    ...
    Backward pass: compute gradients of all control blocks via the adjoint model.
# Arguments
- `state`: output of `unconstrained_global_forward` containing `c`, `n`, and `A`.
